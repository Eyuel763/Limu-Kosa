...
async function bootstrap() {
  ...
  const originsRaw = config.get<string>("CORS_ORIGINS") ?? config.get<string>("CORS_ORIGIN") ?? "http://localhost:3000";
  const origins: boolean | string | string[] = originsRaw === "*" ? true : originsRaw.split(",").map((o) => o.trim());
