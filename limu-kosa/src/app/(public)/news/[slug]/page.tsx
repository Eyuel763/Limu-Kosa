  ...
  if (!article) {
    return (
      <div className="min-h-screen bg-[#F8F6F1] flex flex-col items-center justify-center p-4 text-center">
        <h2 className="text-2xl font-black text-[#2C2C2C]">{tDynamic("Article Not Found", "")}</h2>
        <p className="text-[#50627A] mt-2 max-w-sm">{tDynamic("The article you are searching for does not exist or has been modified.", "")}</p>
        <Link href="/news" className="mt-6 text-sm font-bold text-[#1E5631] underline">{tDynamic("Return to news listing", "")}</Link>
      </div>
    );
  }
      ...
      <div className="bg-[#1E5631] text-white pt-24 pb-20 mb-10 shadow-sm">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <Link href="/news" className="inline-flex items-center gap-2 text-xs font-bold text-white/80 hover:text-white transition uppercase tracking-wider mb-5">
            ...
            {tDynamic("Back to News Updates", "")}
          </Link>
          <DynamicText item={article} field="title" fallback={article.title} className="text-3xl sm:text-5xl font-black tracking-tight leading-tight max-w-5xl break-words" as="h1" />
        </div>
      </div>
          ...
          <div className="bg-white rounded-2xl border border-[#E8E1D4] shadow-sm overflow-hidden p-0">
            ...
            <div className="p-4 border-t border-[#EEF2ED] text-center bg-[#FAF9F6]">
              <p className="text-xs font-bold text-[#50627A] break-words">
                <DynamicText item={article} field="title" fallback={article.title} /> {tDynamic("documentation showcase.", "")}
              </p>
          ...
          <div className="space-y-6 min-w-0 w-full">
            <div className="bg-white rounded-xl border border-[#E8E1D4] shadow-sm p-4 grid grid-cols-2 gap-4 divide-x divide-[#EEF2ED]">
              <div className="flex items-center gap-3 pl-2">
                ...
                <div>
                  <div className="text-[10px] font-black uppercase tracking-wider text-gray-400">{tDynamic("Published", "")}</div>
              ...
              <div className="flex items-center gap-3 pl-4">
                ...
                <div>
                  <div className="text-[10px] font-black uppercase tracking-wider text-gray-400">{tDynamic("Category", "")}</div>
                  <div className="text-xs font-bold text-[#2C2C2C]"><DynamicText item={article} field="category" fallback={article.category ?? "General"} /></div>
                </div>
              </div>
            </div>

            {article.excerpt && (
              <DynamicText item={article} field="excerpt" fallback={article.excerpt} className="text-lg sm:text-xl font-black text-[#1E5631] leading-relaxed tracking-tight break-words" as="h2" />
            )}

            <div className="text-sm sm:text-base leading-8 text-[#50627A] whitespace-pre-wrap font-medium break-words">
              <DynamicText item={article} field="body" fallback={article.body || "No additional detailed body write-up has been provided for this article."} />
            </div>
